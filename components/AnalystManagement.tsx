...
const AnalystManagement: React.FC<AnalystManagementProps> = ({ analysts, clients, onAdd, onUpdate, onRemove }) => {
  ...
  const getClientsForAnalyst = (analystName: string) => {
    return clients.filter(c => 
      c.analystName === analystName || 
      c.analystContemplation === analystName
    );
  };
  ...
  return (
    <div className="animate-fadeIn pb-12">
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-gray-800 tracking-tight uppercase">Gestão de Analistas</h2>
          <p className="text-gray-500 font-medium">Controle de equipe, edição de perfis e acompanhamento de carteiras.</p>
        </div>
                  ...
                  <div key={client.id} className="p-4 hover:bg-blue-50/50 border-b border-gray-50 last:border-0 transition-colors">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-3">
                      <div className="bg-white p-2 rounded-lg border border-gray-100 shadow-sm">
                        ...
                        <p className="text-[10px] font-bold text-gray-700 truncate">{client.analystName}</p>
                      </div>
                      <div className="bg-white p-2 rounded-lg border border-gray-100 shadow-sm">
                        <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Contemplação</p>
                        <p className="text-[10px] font-bold text-gray-700 truncate">{client.analystContemplation}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {!selectedAnalystId ? (
        ...
        <div className="animate-slideUp bg-white rounded-[32px] shadow-sm border border-gray-100 p-10">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                   ...
                   <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase rounded-lg border border-emerald-100">
                     Total na Carteira: {analystClients.length}
                   </span>
                </div>
              </div>
            </div>
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 border-b border-gray-50 pb-2">Clientes Detalhados</h4>
                  ...
                  <tr key={client.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="py-5">
                      <span className={`px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded border ${
                        client.analystName === selectedAnalyst?.name 
                        ? 'bg-blue-50 text-blue-600 border-blue-100' 
                        : 'bg-amber-50 text-amber-600 border-amber-100'
                      }`}>
                        {client.analystName === selectedAnalyst?.name ? 'Cadastro' : 'Contemplação'}
                      </span>
                    </td>
                    <td className="py-5">
                      <div className="flex items-center gap-3">
